...
import { Box, Typography, Paper, Chip, Stack, useTheme, IconButton } from '@mui/material';
...
const KPIItemCard: React.FC<KPIItemCardProps> = ({ kpi, onUpdate }) => {
    ...
    const achievedPercent = kpi.targetValue > 0
    ...
    const emptyStars = Array(5 - stars).fill('☆').join('');

    // Status & Color Logic
    let statusColor: 'default' | 'primary' | 'secondary' | 'error' | 'info' | 'success' | 'warning' = 'warning';
    if (kpi.kpi?.status === 'active') statusColor = 'success';
    else if (kpi.kpi?.status === 'inactive') statusColor = 'default';

    const progressBarColor =
        achievedPercent >= 90 ? 'success.main' :
            achievedPercent >= 60 ? 'warning.main' :
                'error.main';
        ...
        >
            <Box display="flex" justifyContent="space-between" alignItems="flex-start" gap={1}>
                ...
                {onUpdate && (
                    ...
                    >
                        <Box component="img" src={Icons.edit} sx={{ width: 16, height: 16, filter: theme.palette.mode === 'dark' ? 'invert(1)' : 'none' }} />
                    </IconButton>
                )}
            </Box>
            ...
            <Box mb={2}>
                ...
                <Box display='flex' justifyContent='space-between' mt={1}>
                    <Typography variant='caption' color='text.secondary'>
                        Target: <Box component="span" fontWeight="bold" color="text.primary">{kpi.targetValue}</Box>
                    </Typography>
                    <Typography variant='caption' color='text.secondary'>
                        Achieved: <Box component="span" fontWeight="bold" color="text.primary">{kpi.achievedValue}</Box>
                    </Typography>
                </Box>
            </Box>
            ...
            <Box display='flex' alignItems='center' justifyContent='space-between' pt={2} borderTop="1px solid" borderColor="divider">
                <Typography
                    variant='body2'
                ...
                >
                    Score:
                    <Box component="span" sx={{ color: 'warning.main', fontSize: '1.1rem', letterSpacing: 1 }}>
                        {filledStars}
                    </Box>
                    <Box component="span" sx={{ color: 'action.disabled', fontSize: '1.1rem', letterSpacing: 1 }}>
                        {emptyStars}
                    </Box>
                </Typography>
