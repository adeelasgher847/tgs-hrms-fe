...
const AttendanceDepartmentChart: React.FC = () => {
    ...
    // Cache for raw data to avoid re-fetching on filter change
    ...
    const processAttendanceData = (
        ...
        // Aggregate User Stats: UserId -> Set of Days Present
        ...
        if (filterDeptId === 'all') {
            ...
            const deptStats = new Map<string, { presents: number, absents: number, count: number }>();
            ...
            // Iterate all employees to calculate totals for their dept
            emps.forEach(emp => {
                ...
                // Estimate: 30 days - present days = absent days (Simplistic)
            ...
            targetEmps.forEach(emp => {
                ...
                const fullName = (emp.firstName && emp.lastName) ? `${emp.firstName} ${emp.lastName}` : emp.name;
                // Ensure we have the department name for the row
    ...
    // Re-process when filter changes using cached data
    useEffect(() => {
        if (dataCache) {
            processAttendanceData(dataCache.events, dataCache.depts, dataCache.emps, selectedDeptId);
        }
        ...
        const load = async () => {
            ...
            try {
                ...
                while (hasMore && page <= MAX_PAGES) {
                    const res = await attendanceApi.getAllAttendance(page, startStr, endStr, undefined, tenantId);
                    const items = Array.isArray(res.items) ? (res.items as AttendanceEvent[]) : [];
    ...
    const getTitle = () => {
        if (selectedDeptId === 'all') return language === 'ar' ? 'الحضور حسب القسم (آخر 30 يومًا)' : 'Attendance by Department (Last 30 Days)';
        const d = departments.find(x => x.id === selectedDeptId);
        return language === 'ar' ? `الحضور: ${d?.name || ''}` : `Attendance: ${d?.name || ''}`;
    };
                    ...
                    {chartData.length > 0 ? (
                        chartData.map((row, index) => (
                            <TableRow
                                key={index}
                                hover
                                onClick={() => isAllDept && row.departmentId && setSelectedDeptId(row.departmentId)}
                                    ...
                                    {isAllDept ? (
                                        <AppButton
                                            ...
                                            onClick={(e: React.MouseEvent) => {
                                                e.stopPropagation();
                                                if (row.departmentId) setSelectedDeptId(row.departmentId);
                                            }}
                                            sx={{
                        ...
                        <TableRow>
                            <TableCell colSpan={isAllDept ? 4 : 5} align='center'>
                                <Typography color='textSecondary'>
                                    {language === 'ar' ? 'لا توجد بيانات حضور' : 'No attendance data found'}
                                </Typography>
                            </TableCell>
                        </TableRow>
                    )}
