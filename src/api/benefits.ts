...
export async function listBenefits(filters: BenefitFilters): Promise<PagedResult<Benefit>> {
  const { search = '', status = 'all', type = 'all', page = 1, pageSize = 10 } = filters;
  let items = benefitsDb.filter((b) =>
...
export async function createBenefit(input: Omit<Benefit, 'id'>): Promise<Benefit> {
  const newBenefit: Benefit = { id: uuidv4(), ...input };
  benefitsDb.unshift(newBenefit);
  return simulateLatency(newBenefit);
}

export async function updateBenefit(id: string, input: Partial<Omit<Benefit, 'id'>>): Promise<Benefit> {
...
export async function listEmployeeBenefits(employeeId: string): Promise<EmployeeBenefitAssignment[]> {
...
export async function listAllBenefitAssignments(): Promise<EmployeeBenefitAssignment[]> {
