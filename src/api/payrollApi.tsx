...
class PayrollApiService {
  ...
  async getConfig(): Promise<PayrollConfig | null> {
    try {
      const response = await axiosInstance.get<PayrollConfig>(
        `${this.baseUrl}/config`
      );
  ...
  async createConfig(data: CreatePayrollConfigRequest): Promise<PayrollConfig> {
  ...
  async updateConfig(data: CreatePayrollConfigRequest): Promise<PayrollConfig> {
  ...
  async getEmployeeSalary(employeeId: string): Promise<EmployeeSalaryResponse> {
    const response = await axiosInstance.get<EmployeeSalaryResponse>(
      `${this.baseUrl}/salary/${employeeId}`
    );
    return response.data;
  }
  ...
  async getEmployeeSalaryHistory(
    ...
    const response = await axiosInstance.get<EmployeeSalary[]>(
      `${this.baseUrl}/salary/${employeeId}/history`
    );
    return response.data;
  }
  ...
  async getAllEmployeeSalaries(params?: {
    ...
    >(`${this.baseUrl}/salary`, { params: requestParams });
  ...
  async getPayrollPayslip(id: string): Promise<PayrollRecord> {
    const response = await axiosInstance.get<PayrollRecord>(
      `${this.baseUrl}/${id}/payslip`
    );
    return response.data;
  }
            ...
            stat.departmentComparison.forEach(dept => {
              ...
              if (deptName) {
                if (aggregatedDepartmentComparison.has(deptName)) {
                  const existing = aggregatedDepartmentComparison.get(deptName);
                  if (existing) {
...
export const payrollApi = payrollApiService;
