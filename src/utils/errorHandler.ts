...
export function extractErrorMessage(error: unknown): ErrorHandlingResult {
  ...
  if (error && typeof error === 'object' && 'isAxiosError' in error) {
    ...
    switch (axiosError.response?.status) {
      ...
      case 422:
        return {
          message: axiosError.response?.data?.message || 'Validation failed. Please check your input.',
          type: 'error',
          shouldShow: true,
        };
      case 500:
...
export const departmentErrorMessages = {
  globalDepartmentView: 'This is a global department and cannot be modified. You can only view it as a reference.',
  globalDepartmentUpdate: 'Global departments cannot be modified. They are provided as reference templates for your organization.',
  globalDepartmentDelete: 'Global departments cannot be deleted. They are provided as reference templates for your organization.',
...
export const designationErrorMessages = {
  globalDesignationCreate: 'Global departments are read-only reference templates. To add custom designations, please create your own department with the same name.',
  globalDesignationUpdate: 'Global designations are read-only reference templates and cannot be modified.',
  globalDesignationDelete: 'Global designations are read-only reference templates and cannot be deleted.',
  ...
  if (context) {
    ...
    if (error && typeof error === 'object' && 'isAxiosError' in error) {
      ...
      if (axiosError.response?.status === 400) {
        ...
        if (message?.includes('global') && message?.includes('cannot be modified')) {
          return {
            message: message,
            type: 'error',
            shouldShow: true,
 ...
 */
export function isGlobalDepartment(department: { tenantId?: string; id?: string }): boolean {
  // Global departments typically have no tenantId or a specific global tenantId
  return !department.tenantId || department.tenantId === 'GLOBAL' || department.id?.startsWith('global-');
}
 ...
 */
export function isGlobalDesignation(designation: { tenantId?: string; id?: string }): boolean {
  // Global designations typically have no tenantId or a specific global tenantId
  return !designation.tenantId || designation.tenantId === 'GLOBAL' || designation.id?.startsWith('global-');
}
